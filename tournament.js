...
export function validateState(s){
  ...
  for(const team of s.teams){if(typeof team.id!=='string'||!/^[a-zA-Z0-9_-]{1,100}$/.test(team.id)||ids.has(team.id))throw Error('Invalid or duplicate team ID.');ids.add(team.id);if(![1,2,3].includes(Number(team.shift))||typeof team.name!=='string'||!team.name.trim()||team.name.length>150||!Array.isArray(team.bowlers)||team.bowlers.length!==3)throw Error('Invalid team.');let average=0;
    for(const b of team.bowlers){if(typeof b.name!=='string'||!b.name.trim()||b.name.length>150||!number(b.average,0,300)||!['Male','Female','Male Senior','Female Senior'].includes(b.gender))throw Error('Invalid bowler.');const h=Math.min(b.gender==='Male'?35:40,Math.max(0,210-b.average));if(b.handicap!==h)throw Error('Bowler handicap does not match the tournament rule.');average+=b.average;}if(average>630)throw Error('Team average exceeds 630.');
  }
...
function entry(e){if(!e)return null;const result={};for(const k of ['key','name','team','shift','seed','score','handicap','games','scratch'])if(typeof e[k]==='string'||typeof e[k]==='number')result[k]=e[k];return result;}
...
function publicFinals(state){const results={};const fingerprint=JSON.stringify({teams:state.teams,games:state.games.filter(g=>g&&g.number>=1&&g.number<=7).sort((a,b)=>a.number-b.number)});for(const type of ['overall','seniors','women','team']){const e=state.finals?.events?.[type];if(!e)continue;const out={stage:e.stage,outdated:e.source!==fingerprint};for(const field of ['pool','qualifiers','seeded','field','lastSix'])if(Array.isArray(e[field]))out[field]=e[field].map(entry);if(e.scores&&out.qualifiers)out.qualifiers=out.qualifiers.map(q=>({...q,eliminationScratch:typeof e.scores[q.key]==='number'?e.scores[q.key]:null}));if(e.ladder)out.ladder={seeds:e.ladder.seeds.map(entry),matches:e.ladder.matches.map(match)};if(e.champion)out.champion=entry(e.champion);for(const field of ['round16','round8'])if(Array.isArray(e[field]))out[field]=e[field].map(match);results[type]=out;}return results;}
export function publicResults(state){
  const stats=calculate(state),byId=Object.fromEntries(state.teams.map(t=>[t.id,t]));
  return {teams:state.teams.map(t=>({id:t.id,name:t.name,shift:t.shift,bowlers:t.bowlers.map(b=>({name:b.name,gender:b.gender,average:b.average,handicap:b.handicap}))})),standings:stats,finals:publicFinals(state),games:state.games.filter(Boolean).map(g=>({number:g.number,day:g.day,matches:g.matches.filter(m=>byId[m.aId]&&byId[m.bId]).map(m=>({id:m.id,shift:m.shift,laneA:m.laneA,laneB:m.laneB,completed:!!m.completed,a:byId[m.aId].name,b:byId[m.bId].name,bowlers:[0,1,2].map(pos=>({a:byId[m.aId].bowlers[pos].name,b:byId[m.bId].bowlers[pos].name,aHandicap:byId[m.aId].bowlers[pos].handicap,bHandicap:byId[m.bId].bowlers[pos].handicap,aScore:m.completed?m.bowlers.find(b=>b.pos===pos)?.a:null,bScore:m.completed?m.bowlers.find(b=>b.pos===pos)?.b:null}))}))}))};
}
