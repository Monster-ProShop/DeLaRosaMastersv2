...
function publicFinals(state){const results={};const fingerprint=JSON.stringify({teams:state.teams,games:state.games.filter(g=>g&&g.number>=1&&g.number<=7).sort((a,b)=>a.number-b.number)});for(const type of ['overall','seniors','women','team']){const e=state.finals?.events?.[type];if(!e)continue;const out={stage:e.stage,outdated:e.source!==fingerprint,rulesVersion:e.rulesVersion===2?2:1,shiftCount:e.shiftCount===1?1:2};for(const field of ['pool','qualifiers','seeded','field','lastSix','finalResults'])if(Array.isArray(e[field]))out[field]=e[field].map(entry);if(e.scores&&out.qualifiers)out.qualifiers=out.qualifiers.map(q=>({...q,eliminationScratch:typeof e.scores[q.key]==='number'?e.scores[q.key]:null}));if(e.ladder)out.ladder={seeds:e.ladder.seeds.map(entry),matches:e.ladder.matches.map(match)};if(e.champion)out.champion=entry(e.champion);for(const field of ['round10','round16','round8'])if(Array.isArray(e[field]))out[field]=e[field].map(match);results[type]=out;}return results;}
